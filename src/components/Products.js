...
const Products = ({ backgroundColor, titleColor, title, colorSlide }) => {
  const [currentIndex, setCurrentIndex] = useState(0);
  // Número máximo de tarjetas a mostrar
  const [isMobile, setIsMobile] = useState(window.innerWidth <= 1740); // Establecer isMobile basado en el ancho de la ventana
...
// Actualizar la función prevSlide para retroceder solo 2 tarjetas en dispositivos móviles
const prevSlide = () => {
  setCurrentIndex(currentIndex === 0 ? products.length - cardLimit : currentIndex - cardLimit);
};

// Actualizar la función nextSlide para avanzar solo 2 tarjetas en dispositivos móviles
const nextSlide = () => {
  setCurrentIndex(currentIndex === products.length - cardLimit ? 0 : currentIndex + cardLimit);
};

  return (
    <div className='w-full bg-white p-6 mt-10 lg:mt-20 relative h-[400px] lg:h-[600px]'>
      <div className='max-w-[1600px] mx-auto'>
        <h1 className='text-black uppercase font-bold italic text-[26px] lg:text-[36px] leading-8 pt-2 lg:pl-16'>Los más vendidos</h1>
            ...
            <div className="flex absolute bottom-2 left-0 right-0 justify-center">
              {products.map((_, i) => (
                <div
                  ...
                  className={`flex justify-center items-center border border-red-700 relative ml-3 rounded-full cursor-pointer ${currentIndex === i ? 'bg-[#DC1E0F]' : ''}`}
              >
                <RxDotFilled className={`transition-all fill-none h-[8px] w-[8px] ${currentIndex === i ? 'text-[#DC1E0F]' : 'text-white'}`} />
                </div>
              ))}
            </div>
          ) : (
